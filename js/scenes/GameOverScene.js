...
class GameOverScene extends Phaser.Scene {
    ...
    create() {
        ...
        if (this.stats.won && this.stats.nextNight) {
        ...
        } else if (this.stats.won && !this.stats.nextNight) {
            const finalButton = this.add.rectangle(buttonX2, 650, 180, 60, 0x00ff00).setInteractive();
            this.add.text(buttonX2, 650, '¡GANASTE!', { fontSize: '22px', fill: '#000000', fontStyle: 'bold' }).setOrigin(0.5);
            finalButton.on('pointerdown', () => this.scene.start('MenuScene'));
        } else {
            ...
            retryButton.on('pointerdown', () => this.scene.start('GameScene', { night: this.stats.night, nightName: 'Noche ' + this.stats.night }));
        }
    }
