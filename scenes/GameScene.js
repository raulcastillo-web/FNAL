class GameScene extends Phaser.Scene {
    ...
    createMathEntities() {
        const entities = [
            { name: 'La Suma Académica', emoji: '📚' },
            { name: 'El Multiplicador', emoji: '📋' },
            { name: 'La División', emoji: '🧮' },
            { name: 'El Profesor', emoji: '📐' },
            { name: 'La Función', emoji: '📊' }
        ];

        let xPos = 100;
        entities.forEach((entity, index) => {
            const color = [0xff0000, 0xff6600, 0xffff00, 0x00ffff, 0xff00ff][index];
            const mathEntity = new MathEntity(
                this,
                xPos,
                100,
    ...
    updateEntities() {
        this.mathEntities.forEach(entity => entity.update());
    }
